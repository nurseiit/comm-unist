...
You will implement Dijkstra's Algorithm for finding the shortest route
...
*  `calcRouteDistance`: calculate total distance of a route, which is a list of
   airport names.
*  `findShortestRoute`: return one shortest route between a given origin
   airport and destination airport.

In `findShortestRoute` function, you will return a route in form of a list of string from origin to destination.
...
# Notes 
The input files will be in these forms:
...
* [test-cases/0-dijkstra.txt](https://class.unicss.org/cse221-2019-spring/lab6-prep/blob/master/test-cases/0-dijkstra.txt):
...
* [test-cases/0-output-dijkstra.txt](https://class.unicss.org/cse221-2019-spring/lab6-prep/blob/master/test-cases/0-output-dijkstra.txt):
   This is the output for the task 2 which includes shortest distances from dijkstra.txt
...
the pre-grading schedule separately. 
